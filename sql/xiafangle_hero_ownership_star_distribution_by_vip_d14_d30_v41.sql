select      row_number() over(
                order by q.period_sort,q."历史VIP",q."英雄名称",q.star_sort
            ) "序号",
            q."统计周期",
            q."历史VIP",
            q."英雄名称",
...
(
    ...
    (
        select      p.period_sort,
                    p."统计周期",
                    cast(p.vip_level as bigint) "历史VIP",
                    count(distinct p.account_id) "VIP玩家数"
        from
        (
    ...
    (
        select      sc.period_sort,
                    ...
                    sc."该星级人数",
                    sum(sc."该星级人数") over(
                        partition by sc.period_sort,sc."历史VIP",sc."英雄名称"
                    ) "英雄持有人数"
        from
        (
