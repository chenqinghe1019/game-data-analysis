SELECT
    ...
    round(
        x.report_npc_hp_max
        / NULLIF(x.svr_npc_hp_max, 0),
        4
    ) AS "怪物异常生命比例（客户端/服务端）",

    round(
        x.report_hp_max
        / NULLIF(x.svr_hp_max, 0),
        4
    ) AS "城墙异常生命比例（客户端/服务端）",
...
(
    ...
    (
        SELECT
            CAST(
                login_e."#account_id" AS varchar
            ) AS role_id,

            CAST(
                login_e."$part_date" AS date
            ) AS login_date,

            max_by(
                TRY_CAST(
                    login_e."season" AS bigint
                ),
                CAST(
                    login_e."#event_time" AS timestamp
                )
            ) FILTER
            (
                WHERE TRY_CAST(
